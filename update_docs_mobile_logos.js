...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: "Rubik", sans-serif; }
    ...
    body { background-color: var(--bg-base); color: var(--text-main); font-size: 15px; line-height: 1.6; -webkit-font-smoothing: antialiased; display: flex; flex-direction: column; min-height: 100vh; overflow-x: hidden; }
    ...
    /* GLOBAL NAVBAR */
    .navbar { height: var(--navbar-height); width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 0 24px; background: var(--bg-panel); border-bottom: 1px solid var(--border-color); z-index: 100; position: sticky; top: 0; }
    ...
    /* LAYOUT CONTAINER */
    .layout-container { display: flex; flex: 1; position: relative; align-items: stretch; }

    /* SIDEBAR */
    .sidebar { width: var(--sidebar-width); border-right: 1px solid var(--border-color); display: flex; flex-direction: column; background: var(--bg-base); z-index: 50; flex-shrink: 0; position: sticky; top: var(--navbar-height); height: calc(100vh - var(--navbar-height)); overflow-y: auto; }
    ...
    .nav-item-name { font-weight: 600; font-size: 14px; color: var(--text-main); margin-bottom: 4px; display: flex; align-items: center; gap: 8px; }
    .nav-item-path { font-family: Monaco, monospace; font-size: 11px; color: var(--text-muted); display: flex; align-items: center; gap: 8px; }
    .nav-item-icon { width: 16px; height: 16px; object-fit: contain; border-radius: 2px; }
    ...
    .top-bar-docs { display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; padding: 16px 40px; background: var(--bg-panel); border-bottom: 1px solid var(--border-color); z-index: 20; gap: 16px; }
    .sidebar-toggle-btn { display: none; background: var(--bg-glass); border: 1px solid var(--border-color); color: var(--text-main); padding: 12px; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; width: 100%; text-align: center; }
    
    .api-key-container { display: flex; flex-direction: column; gap: 8px; flex: 1; max-width: 500px; width: 100%; }
    .api-key-setup { display: flex; align-items: center; gap: 12px; background: var(--bg-base); border: 1px solid var(--border-color); padding: 10px 16px; border-radius: 8px; transition: border-color 0.2s; width: 100%; }
    ...
    .content-grid { display: grid; grid-template-columns: 1fr 450px; gap: 40px; padding: 40px; width: 100%; align-items: start; }

    .doc-section { display: flex; flex-direction: column; gap: 32px; min-width: 0; }
    .endpoint-header h1 { font-size: 32px; font-weight: 600; margin-bottom: 12px; line-height: 1.2; color: #ffffff; display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
    .endpoint-header h1 img { height: 32px; object-fit: contain; }
    ...
    .param-item { padding-bottom: 16px; border-bottom: 1px solid var(--border-color); display: flex; flex-direction: column; gap: 8px; }
    .param-item:last-child { border-bottom: none; padding-bottom: 0; }
    .param-name { font-family: Monaco, monospace; font-weight: 600; color: #ffffff; font-size: 14px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
    ...
    .param-desc { color: var(--text-muted); font-size: 14px; line-height: 1.5; }
    .param-example { display: inline-block; font-family: Monaco, monospace; font-size: 12px; color: var(--text-main); background: var(--bg-base); padding: 6px 10px; border-radius: 6px; border: 1px solid var(--border-color); word-break: break-all; }

    /* CODE INTEGRATION SNIPPETS */
    .code-snippets { background: var(--bg-panel); border: 1px solid var(--border-color); border-radius: var(--radius-lg); overflow: hidden; margin-top: 16px; }
    .code-tabs { display: flex; border-bottom: 1px solid var(--border-color); background: var(--bg-base); overflow-x: auto; }
    .code-tab { padding: 12px 20px; color: var(--text-muted); font-size: 14px; font-weight: 600; cursor: pointer; border-bottom: 2px solid transparent; transition: all 0.2s; white-space: nowrap; }
    ...
    /* PROVIDERS GRID */
    .providers-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 16px; margin-top: 16px; }
    .provider-card { background: var(--bg-base); border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 16px; display: flex; flex-direction: column; align-items: center; gap: 12px; transition: 0.2s; text-align: center; }
    .provider-card:hover { border-color: var(--accent-primary); transform: translateY(-2px); }
    .provider-card img { height: 32px; object-fit: contain; }
    .provider-card span { font-size: 13px; font-weight: 600; color: var(--text-main); }

    /* INTERACTIVE PANEL */
    .interactive-panel { background: var(--bg-panel); border: 1px solid var(--border-color); border-radius: var(--radius-lg); position: sticky; top: calc(var(--navbar-height) + 20px); display: flex; flex-direction: column; }
    ...
    .input-group input { background: var(--bg-base); border: 1px solid var(--border-color); border-radius: var(--radius-sm); padding: 12px 14px; color: var(--text-main); font-family: Monaco, monospace; font-size: 14px; width: 100%; }
    .input-group input:focus { border-color: var(--accent-primary); outline: none; }
    
    .btn-send { background: var(--text-main); color: var(--bg-base); border: none; border-radius: var(--radius-sm); padding: 16px; font-weight: 700; font-size: 14px; cursor: pointer; text-transform: uppercase; width: 100%; margin-top: 8px; transition: 0.2s; }
    .btn-send:hover { background: #e0e0e0; }
    .btn-send:disabled { opacity: 0.7; cursor: not-allowed; }
    
    .response-area { border-top: 1px solid var(--border-color); background: var(--bg-base); display: flex; flex-direction: column; border-radius: 0 0 var(--radius-lg) var(--radius-lg); }
    ...
    .res-body { padding: 16px; margin: 0; font-family: Monaco, monospace; font-size: 12px; color: #ffffff; overflow-x: auto; max-height: 400px; overflow-y: auto; }
    ...
    @media (max-width: 768px) {
      /* Navbar */
      .navbar { padding: 0 16px; position: sticky; top: 0; z-index: 100; }
      .navbar-toggle { display: flex; }
      .nav-links { display: none; position: absolute; top: var(--navbar-height); left: 0; right: 0; background: var(--bg-panel); flex-direction: column; padding: 20px; border-bottom: 1px solid var(--border-color); gap: 16px; box-shadow: 0 10px 20px rgba(0,0,0,0.5); z-index: 99; align-items: flex-start; }
      ...
      /* Sidebar */
      .sidebar { position: fixed; left: -100%; top: var(--navbar-height); bottom: 0; height: calc(100vh - var(--navbar-height)); z-index: 90; transition: left 0.3s; box-shadow: 4px 0 24px rgba(0,0,0,0.5); width: 280px; }
      .sidebar.open { left: 0; }
      
      /* Top Bar */
      .top-bar-docs { padding: 16px; flex-direction: column; align-items: stretch; position: relative; top: 0; }
      .sidebar-toggle-btn { display: block; }
      
      /* Content Grid */
      .content-grid { padding: 16px; }
      .endpoint-header h1 { font-size: 24px; }
      .endpoint-header h1 img { height: 24px; }
      ...
      /* Res Body */
      .res-body { font-size: 11px; padding: 12px; }
    }
  ...
  <div class="layout-container">
    ...
    <main class="main-content" id="mainContent">
      <div class="top-bar-docs">
        <button class="sidebar-toggle-btn" onclick="toggleSidebar()">☰ Menu & Endpoints</button>
    ...
    const endpoints = [
      {
        ...
        items: [
          { 
            ...
            needsAuth: true, isGlobal: true, queryParams: [{name: "page", example: "1", desc: "Nomor halaman (1-100)"}] 
          },
          { 
            ...
            needsAuth: true, isGlobal: true, queryParams: [{name: "q", required: true, example: "ceo", desc: "Kata kunci pencarian"}, {name: "page", example: "1"}] 
          },
          { 
            ...
            needsAuth: true, 
            pathParams: [{name: "platform", required: true, example: "netshort", desc: "Pilih platform target (contoh: melolo, netshort, dll)"}], 
      ...
      },
      {
        ...
        items: [
          { id: "sys-platforms", method: "GET", path: "/api/platforms", name: "Available Platforms", desc: "Daftar lengkap platform scraping yang bisa diakses beserta detailnya.", needsAuth: true, showProviders: true },
          { id: "sys-me", method: "GET", path: "/api/me", name: "Account Profil", desc: "Cek informasi akun, paket, dan sisa limit harian.", needsAuth: true },
          { id: "sys-status", method: "GET", path: "/api/status", name: "System Status", desc: "Cek kesehatan status layanan server scraper." }
        ]
      }
    ];
    ...
    function buildSampleUrl(item) {
      ...
      if (item.pathParams) {
        item.pathParams.forEach(p => { 
          const val = document.getElementById(\`input_path_\${p.name}\`)?.value || p.example || \`\${p.name}\`;
          url = url.replace(\`{\${p.name}}\`, val); 
        });
      }
      if (item.queryParams && item.queryParams.length > 0) {
        const queryStrings = item.queryParams.map(p => {
          const val = document.getElementById(\`input_query_\${p.name}\`)?.value || p.example || '';
...
print(response.json())\`;
    ...
    function updateDynamicLogo() {
      ...
      if (currentEndpoint.pathParams && currentEndpoint.pathParams.some(p => p.name === 'platform')) {
        const platInput = document.getElementById('input_path_platform');
        if (platInput) {
          const val = platInput.value.trim().toLowerCase();
          const logoUrl = getProviderLogo(val);
          if (logoUrl) {
            titleEl.innerHTML = \`<img src="\${logoUrl}" alt="\${val}"> \${currentEndpoint.name}\`;
          } else {
            titleEl.textContent = currentEndpoint.name;
          }
        }
      }
    }
    ...
    function selectEndpoint(item) {
      ...
      // Render Dynamic Header (Logo if platform is specific)
      let headerImg = '';
      if (item.pathParams && item.pathParams.some(p => p.name === 'platform') && item.pathParams[0].example) {
        const logoUrl = getProviderLogo(item.pathParams[0].example.toLowerCase());
        if(logoUrl) headerImg = \`<img src="\${logoUrl}" alt="\${item.pathParams[0].example}"> \`;
      }
      if (item.isGlobal || item.showProviders) {
        headerImg = \`<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--accent-primary)" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg> \`;
      }

      let html = \`
        <div class="endpoint-header">
          <h1 id="endpointTitle">\${headerImg}\${item.name}</h1>
          <p>\${item.desc}</p>
        </div>
      ...
      // Render Providers Grid if applicable
      if (item.isGlobal || item.showProviders) {
        let gridHtml = \`<div class="params-section" style="margin-top:24px">
          <h3>🌐 ${PROVIDERS.length} Supported Providers</h3>
      ...
      // Update Snippets after inputs are rendered
    ...
    async function executeRequest() {
      ...
      let url = buildSampleUrl(currentEndpoint);
...
console.log('Successfully applied all mobile responsive fixes and platform logos.');
