...
module.exports = {
  platform: 'klikfilm',
  baseUrl: 'https://klikfilm.com/v3/mobile',

  buildUrl: (query) => {
    let url = 'https://klikfilm.com/v3/mobile/movies';

    if (query.category) {
      url += `?genre=${encodeURIComponent(query.category)}`;
    }
  ...
  parseData: (html) => {
    ...
    $('.movie-item, .thumbnail, a[href*="/movie/"]').each((i, el) => {
      if (i >= 20) return;

      const $el = $(el);
      const anchor = $el.is('a') ? $el : $el.find('a').first();
      const img = $el.find('img').first();
      const titleEl = $el.find('.title, h4');

      const title = titleEl.text().trim() || img.attr('alt') || anchor.attr('title') || 'No Title';
      
      let sourceUrl = anchor.attr('href') || '';
      if (sourceUrl && !sourceUrl.startsWith('http')) {
        sourceUrl = 'https://klikfilm.com/v3/mobile' + (sourceUrl.startsWith('/') ? sourceUrl : '/' + sourceUrl);
      }
